...
class DEBLUR(object):
    ...
    def input_producer(self, batch_size=10):
        def read_data():
            img_a = tf.image.decode_image(tf.compat.v1.read_file(tf.compat.v1.string_join(['./training_set/', self.data_queue[0]])),
                                          channels=3)
            img_b = tf.image.decode_image(tf.compat.v1.read_file(tf.compat.v1.string_join(['./training_set/', self.data_queue[1]])),
                                          channels=3)
        ...
        def preprocessing(imgs):
            ...
            img_crop = tf.unstack(tf.compat.v1.random_crop(tf.stack(imgs, axis=0), [2, self.crop_size, self.crop_size, self.chns]),
                                  axis=0)
            return img_crop
        ...
        with tf.compat.v1.variable_scope(scope, reuse=reuse):
            with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
                ...
                for i in xrange(self.n_levels):
                    ...
                    inp_pred = tf.compat.v1.stop_gradient(tf.compat.v1.image.resize_images(inp_pred, [hi, wi], method=0))
                    inp_all = tf.concat([inp_blur, inp_pred], axis=3, name='inp')
                    if self.args.model == 'lstm':
                        rnn_state = tf.compat.v1.image.resize_images(rnn_state, [hi // 4, wi // 4], method=0)
